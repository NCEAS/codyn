...
test_that("variance_ratio function returns correct result", {
    ...
    myresults4<-variance_ratio(dat1, replicate=NA, bootnumber=10)
    
    #test that works with one, specified replicate
    myresults5<-variance_ratio(dat1,replicate="subplot", bootnumber=10)
    
    #test that works with replicate as a character
    myresults6<-variance_ratio(dat3,replicate="subplot", bootnumber=10)
    
    #test that gives same for a single rep if average reps is false
    myresults7<-variance_ratio(dat3, bootnumber=1, replicate="subplot", average.replicates=FALSE)
