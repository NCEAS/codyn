...
RAC_difference_test <- function(df, time.var = NULL, species.var, 
  ...
  ranktog_split <- lapply(ranktog_split,
                          FUN = SERSp2, "rank", "rank2", abundance.var, paste(abundance.var, 2, sep = ""))
...
SERSp2 <- function(df, rank.var, rank.var2, abundance.var, abundance.var2){
  
  #rank.var2 <- paste(rank.var, 2, sep = '')
  ...
  e_t1 <- S(df[[abundance.var]])
  s_t1 <- EQ(as.numeric(df[[abundance.var]]))
