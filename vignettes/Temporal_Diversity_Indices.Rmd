...
The metric was introduced by MacArthur and Wislon [-@MacArthur1963] and modified by Diamond (1969) to be expressed as a proportion in order to compare turnover between sites that differ in species richness.
...
Rank clocks highlight that in the Konza grasslands there has been tremendous re-ordering in the relative abundance of dominant speices, especially with big bluestem  compared to little bluestem over time in the annually burned site but not the unburned site.
```{r, fig.width = 7, fig.height = 5}

aggdat <- aggregate(abundance ~ species * year * replicate, 
                    data = subset(collins08, species == "andrgera" | species == "andrscop" | species == "poaprat" | species == "sorgnuta"), 
                    FUN = mean)

ggplot(aggdat, aes(year, abundance, color = species)) + 
  geom_line(size = 2) + coord_polar() + theme_bw() + facet_wrap(~replicate) +
  ggtitle("Dominant species in \n Annually burned vs unburned plots, Konza")
```
...
```{r fig.width = 7, fig.height = 4}
# Create a column with the final year from the returned year_pair
KNZ_rankshift$year<-as.numeric(substr(KNZ_rankshift$year_pair, 6, 9))
...
The rate_change function assesses the rate and pattern of variability within a community, namely illustrating how whether species reordering over time is leading to directional change. This function calculates differences in species composition between samples at increasing time intervals. First, a dissimilarity matrix is calculated using Euclidean distance. Then, the Euclidean distance values are plotted against increasing time lag intervals. For example, a data set with 6 time intervals will have 5 one-year time lags (year 1 vs. year 2, year 2 vs. year 3 ...) and 4 two-year time lags (year 1 vs. year 3, year 2 vs. year 4 ...). Finally, distance values are regressed against time lag interval. The slope of the regression line indicates the rate and direction of change. A positive slope indicates directional change, a flat slope indicates stability and a negative slope implies convergence toward an earlier sampling period.

```{r}
    rate.res <- rate_change(collins08, replicate.var= "replicate", 
                    time.var= "year", species.var= "species", abundance.var= "abundance")
```
```{r, echo = F}
...
The annually burned grassland plots shown here have relatively greater directional change through time, which can also be observed in the rank clocks. For example, the dominant species _Andropogon gerardii_ decreases substantially over time in the annually burned plot, yet stays stable in the unburned plot.
