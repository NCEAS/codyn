...
**`variance_ratio`** which characterizes species covariance [@schluter1984; @houlahan2007], and includes a null-modeling approach to test significance [@hallett2014].

**`synchrony`** which compares the variance of the aggregated community with the variance of individual components [@loreau2008].

**`GROSS et al 2014 METRIC`** which compares the average correlation of each individual species with the rest of the aggregated community [@gross2014].
...
#Community stability
The `community_stability` function aggregates species abundances within replicate and year, and used these values to calculate community stability as µ/σ (i.e., the inverse of the coefficient of variation) [@tilman1999]. It includes an argument to calculate community stabilty across multiple replicates, and will return a dataframe with the name of the replicate column and the stability value.
```{r results='asis'}
...
## Synchrony Option 2
Gross et al. 2014 developed a metric of synchrony that compares the average average correlation of each individual species with the rest of the aggregated community:
