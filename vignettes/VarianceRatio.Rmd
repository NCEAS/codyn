...
- **`synchrony`** compares the variance of the aggregated community with the variance of individual components [@loreau2008].

- **`GROSS et al 2014 METRIC`** compares the average correlation of each individual species with the rest of the aggregated community [@gross2014].
...
A second criticism of the variance ratio is that it is sensitive to species richness. This is a particular concern when the metric is used to compare communities that have different levels of species richness. The most conservative approach is to restrict use of the variance ratio to two-species communities [@hector2010]. Two alternative metrics quantifying species asynchrony have been developed in part to respond to this issue.
...
This measure of synchrony is standardized from -1 (perfect asynchrony) to 1 (perfect synchrony) and is centered at 0 when species fluctuate independently. 
