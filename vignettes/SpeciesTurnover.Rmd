...
If there are no replicates in the dataset, specify `replicate=NA`. To illustrate, calculate turnover on a subset of the KNZ dataset that only includes one replicate subplot.
```{r results='asis'}
...
Total turnover incorporates both species appearances and disappearances, but sometimes it is useful to parse their relative contribution. For example, a timepoint in which many species appear may tell a different ecological story than a timepoint in which many species drop from the system, but the total turnover in both scenarios may be similar. 
...
```{r results='asis'}
    KNZ_appearance<-turnover(KNZ, replicate="subplot", species="species", year="year", abundance="abundance", metric="appearance")
    kable(head(KNZ_appearance))
```
...
```{r results='asis'}
    KNZ_disappearance<-turnover(KNZ, replicate="subplot", species="species", year="year", abundance="abundance", metric="disappearance")
kable(head(KNZ_disappearance))
```
