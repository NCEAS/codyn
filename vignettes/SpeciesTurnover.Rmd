...
library(codyn)
data(collins08)
KNZ_turnover<-turnover(df=collins08, replicate.var="replicate", species.var="species", time.var="year", abundance.var="abundance")
```
```{r echo=FALSE}
...
Note that if `replicate=NA` but the dataset in fact includes replicates, `turnover` will calculate across all species present in a given year (i.e., it will return a site-level measure of turnover). For example, specifying `replicate=NA` while using the entire `collins08` dataset yields total turnover across the unburned and burned locations combined.

```{r results='asis'}
KNZ_turnover_agg<-turnover(df=collins08, replicate.var=as.character(NA), species.var="species", time.var="year", abundance.var="abundance")
```
```{r echo=FALSE}
...
```{r results='asis'}
KNZ_appearance<-turnover(df=collins08, replicate.var="replicate", species.var="species", time.var="year", abundance.var="abundance", metric="appearance")
```
...
```{r results='asis'}
KNZ_disappearance<-turnover(df=collins08, replicate.var="replicate", species.var="species", time.var="year", abundance="abundance", metric="disappearance")
```
...
```{r}
KNZ_rankshift <- meanrankshift(df=collins08, replicate.var="replicate", time.var="year", species.var="species", abundance.var="abundance")
...
If`replicate=NA` but the dataset in fact includes replicates, `meanrankshifts` will be calculated using all species present in the datset. For collins08, this would return meanrankshifts for the burned and unburned areas combined.
```{r}
KNZ_rankshift_agg <- meanrankshift(df=collins08, replicate.var=as.character(NA), time.var="year", species.var="species", abundance.var="abundance")
#Select the final year from the returned year_pair
KNZ_rankshift$year<-as.numeric(substr(KNZ_rankshift$year_pair, 6,9))
```
...
```{r}
##plotting
ggplot(KNZ_rankshift, aes(year, MRS, color=replicate)) + geom_line() + theme_bw()+ theme(legend.position="none")
```
