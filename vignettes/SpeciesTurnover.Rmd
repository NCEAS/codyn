...
library(ggplot2)
ggplot(KNZ_turnover, aes(x=year, y=total, color=replicate)) + geom_line(size = 2) + theme_bw() #+ theme(legend.position="none")
```
...
# Species rank shifts
Species rank shifts describe within-community reording among species over time [@Collins2008]. 

## Rank clocks: visualizing species rank shifts

Rank clocks plot the rank order of abundance of each species over time in a circle, starting with a vertical axis at 12 o'clock. They are useful to visualize the degree to which species fluctuate over time [@Collins2008]. 
...
ggplot(dat, aes(x,value,color=variable)) + geom_line(size = 2) + theme_bw()
```
...
Mean rank shifts describe relative changes in species rank abundances, which indicate the degree of species reording over time. This metric is calculated as:
