...
```{r}
KNZ_rankshift <- meanrankshift(df=collins08, replicate.var="replicate", abundance.var="abundance")
...
If`replicate.var=NA` but the dataset in fact includes replicates, `meanrankshifts` will be calculated using all species present in the datset. For collins08, this would return meanrankshifts for the burned and unburned areas combined.
```{r}
KNZ_rankshift_agg <- meanrankshift(df=collins08, replicate.var=NA)
#Select the final year from the returned year_pair
KNZ_rankshift$year<-as.numeric(substr(KNZ_rankshift$year_pair, 6, 9))
```
